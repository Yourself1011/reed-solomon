...
export const polyLongDiv = (dividend: GFNumber[], divisor: GFNumber[]) => {
    ...
    steps.push(
        [...divisor]
            .map((d) => d.mult(multi))
            .concat(
                Array(dividend.length - divisor.length).fill(new GFNumber(0)),
            ),
    );
    ...
    for (let i = 1; i < dividend.length - (divisor.length - 1); i++) {
        ...
        steps.push(
            [...Array(i).fill(new GFNumber(0)), ...divisor]
                .map((d) => d.mult(multi))
                .concat(
                    Array(dividend.length - divisor.length - i).fill(
                        new GFNumber(0),
                    ),
                ),
        );
        diffs.push(
            [...diffs[diffs.length - 1]].map((n, i) =>
                n.sub(steps[steps.length - 1][i]),
            ),
        );
        // console.log(diffs);
    }
    ...
    return {
        ...
        diffs,
        quotient,
        remainder: diffs[diffs.length - 1].slice(
            dividend.length - divisor.length + 1,
        ),
    };
};
...
export const polyText = (
    ...
    return p
        .map((n, i) =>
            (start && i < start) ||
            (end && i >= end) ||
            (skipZero && n.valueOf() == 0)
                ? ""
                : ((typeof n === "string" || n.valueOf() >= 0) &&
                  i != (start ?? 0)
                      ? "+"
                      : "") +
                  (i == p.length - 1
...
export const evalPoly = (p: GFNumber[], x: GFNumber) => {
    ...
    return new GFNumber(Math.round(sum.value * 10 ** 4) / 10 ** 4);
};
