  ...
  return (
    <>
      <main className="p-8">
        ...
        <p className="mb-4">
          Problem: How can we add things to a wireless message, such that, if it
          gets tampered with, through, for example, interference, we can detect
          and correct those errors, without needing to send a copy of the
          message? We'll achieve this by adding some amount of redundant
          characters to the message. This number is set in advance for both the
          encoding and decoding side. This algorithm can correct 1 error for
          every 2 redundant characters. Positions are 0-indexed (start at 0).
        </p>
        <p className="mb-4">
          Because we're working with computers, we don't want our numbers to get
          too large, since that means we need to send more data unnecessarily.
          To accomplish this, we will use a number system known as{" "}
          <MathJax inline>{"\\(GF(2^8)\\)"}</MathJax>, which keeps the results
          of all arithmetic operations between 0 and 256, meaning we can store
          it in a byte, while still following the basic rules of each operation.
          Here is a calculator for this number system.
        </p>
        <GFCalc />
        <p className="mb-4">
          Note that addition and subtraction do the same thing, and are
        ...
        {on ? (
          <>
            ...
            <p className="mb-2">
              Corrupt some of these numbers, but keep them below 256. Since
              there are {redundantCharacters} redundant characters, our
              algorithm can fix up to {Math.floor(redundantCharacters / 2)}{" "}
              errors.
            </p>
            <form
